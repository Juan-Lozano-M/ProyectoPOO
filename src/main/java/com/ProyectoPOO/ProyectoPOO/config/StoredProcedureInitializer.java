...
public class StoredProcedureInitializer implements ApplicationRunner {
    ...
    private static final String SP_CARGUE_TRAYECTO = """
            CREATE PROCEDURE sp_cargue_trayecto(
                ...
                IN p_persona_identification VARCHAR(30),
                IN p_vehicle_plate VARCHAR(6),
                IN p_route_code VARCHAR(50),
                IN p_location TEXT,
                IN p_stop_order VARCHAR(20),
                IN p_latitude VARCHAR(30),
                IN p_longitude VARCHAR(30),
                IN p_registered_by_login VARCHAR(50)
            )
            BEGIN
    ...
    private static final String SP_VALIDAR_TRAYECTO = """
            ...
            BEGIN
                DECLARE v_id BIGINT;
                DECLARE v_persona_identification VARCHAR(30);
                DECLARE v_vehicle_plate VARCHAR(6);
                DECLARE v_route_code VARCHAR(50);
                DECLARE v_location TEXT;
                DECLARE v_stop_order VARCHAR(20);
                DECLARE v_latitude VARCHAR(30);
                DECLARE v_longitude VARCHAR(30);

                DECLARE v_persona_id BIGINT;
                DECLARE v_vehicle_id BIGINT;
                DECLARE v_person_type VARCHAR(1);
                DECLARE v_driver_state VARCHAR(2);
                DECLARE v_doc_count INT;
                DECLARE v_doc_not_enabled INT;
                DECLARE v_observacion TEXT;
                DECLARE v_stop_order_num INT;
                DECLARE v_lat_num DECIMAL(20,10);
                DECLARE v_lon_num DECIMAL(20,10);

                SET v_id = (SELECT MIN(id) FROM trayecto_tmp WHERE id_cargue = p_id_cargue AND estado = 'CARGADO');

                read_loop: WHILE v_id IS NOT NULL DO
                    ...
                    SET v_person_type = NULL;
                    SET v_driver_state = NULL;
                    ...
                    IF v_persona_id IS NULL THEN
                        SET v_observacion = CONCAT(v_observacion, 'Conductor no encontrado por identificacion; ');
                    ELSEIF v_person_type <> 'C' THEN
                        SET v_observacion = CONCAT(v_observacion, 'La persona no es un conductor (tipo C); ');
                    END IF;

                    SELECT id INTO v_vehicle_id FROM vehicles WHERE plate = v_vehicle_plate LIMIT 1;
                    IF v_vehicle_id IS NULL THEN
                        SET v_observacion = CONCAT(v_observacion, 'Vehiculo no encontrado por placa; ');
                    END IF;
                    ...
                    IF v_persona_id IS NOT NULL AND v_vehicle_id IS NOT NULL THEN
                        ...
                        IF v_driver_state IS NULL THEN
                            SET v_observacion = CONCAT(v_observacion, 'El conductor no esta asociado al vehiculo; ');
                        ELSEIF v_driver_state <> 'PO' THEN
                            SET v_observacion = CONCAT(v_observacion, 'El conductor no puede operar el vehiculo (estado ', v_driver_state, '); ');
                        END IF;
                    END IF;
                    ...
                    IF v_vehicle_id IS NOT NULL THEN
                        ...
                        IF v_doc_count = 0 OR v_doc_not_enabled > 0 THEN
                            SET v_observacion = CONCAT(v_observacion, 'No todos los documentos del vehiculo estan HABILITADO; ');
                        END IF;
                    END IF;

                    IF v_route_code IS NULL OR TRIM(v_route_code) = '' THEN
                        SET v_observacion = CONCAT(v_observacion, 'El codigo de ruta es obligatorio; ');
                    END IF;
                    IF v_location IS NULL OR TRIM(v_location) = '' THEN
                        SET v_observacion = CONCAT(v_observacion, 'La ubicacion es obligatoria; ');
                    END IF;

                    IF v_stop_order IS NULL OR TRIM(v_stop_order) = '' OR NOT (v_stop_order REGEXP '^[0-9]+$') THEN
                        SET v_observacion = CONCAT(v_observacion, 'El orden de parada debe ser un numero entero >= 0; ');
                    ELSE
                        SET v_stop_order_num = CAST(v_stop_order AS UNSIGNED);
                        IF v_stop_order_num > 5 AND v_stop_order_num <> 99 THEN
                            SET v_observacion = CONCAT(v_observacion, 'Maximo 5 paradas intermedias permitidas (orden 0-5 o 99 para final); ');
                        END IF;
                    END IF;

                    IF v_latitude IS NOT NULL AND TRIM(v_latitude) <> '' THEN
                        IF NOT (v_latitude REGEXP '^-?[0-9]+(.[0-9]+)?$') THEN
                            SET v_observacion = CONCAT(v_observacion, 'La latitud no es un numero valido; ');
                        ELSE
                            SET v_lat_num = CAST(v_latitude AS DECIMAL(20,10));
                            IF v_lat_num < -90 OR v_lat_num > 90 THEN
                                SET v_observacion = CONCAT(v_observacion, 'La latitud debe estar entre -90 y 90; ');
                            END IF;
                        END IF;
                    END IF;

                    IF v_longitude IS NOT NULL AND TRIM(v_longitude) <> '' THEN
                        IF NOT (v_longitude REGEXP '^-?[0-9]+(.[0-9]+)?$') THEN
                            SET v_observacion = CONCAT(v_observacion, 'La longitud no es un numero valido; ');
                        ELSE
                            SET v_lon_num = CAST(v_longitude AS DECIMAL(20,10));
                            IF v_lon_num < -180 OR v_lon_num > 180 THEN
                                SET v_observacion = CONCAT(v_observacion, 'La longitud debe estar entre -180 y 180; ');
                            END IF;
                        END IF;
                    END IF;
    ...
    private static final String SP_PROCESAR_TRAYECTO = """
            ...
            BEGIN
                ...
                DECLARE v_latitude VARCHAR(30);
                DECLARE v_longitude VARCHAR(30);
                DECLARE v_registered_by_login VARCHAR(50);
                DECLARE v_persona_id BIGINT;
                DECLARE v_vehicle_id BIGINT;

                SET v_id = (SELECT MIN(id) FROM trayecto_tmp WHERE id_cargue = p_id_cargue AND estado = 'VALIDADO');

                proc_loop: WHILE v_id IS NOT NULL DO
